...
const documentTypes = [
  {
    ...
    icon: FileText,
    description: "Generate detailed technical specifications with structured formatting and clarity.",
    color: "bg-blue-500",
...
- [DIAGRAM] placeholders where needed`
  },
  {
    ...
    icon: FileCode,
    description: "Create comprehensive SRS documents outlining system functionalities and requirements.",
    color: "bg-purple-500",
...
Format using Markdown with clear headings and bullet points.`
  },
  {
    ...
    icon: Network,
    description: "Design and document system architectures with clear diagrams and structured descriptions.",
    color: "bg-green-500",
...
const GeneratePage = () => {
  ...
  const selectedDocument = documentTypes.find((type) => type.id === selectedType);
      ...
      const response = await axios.post(
        "https://api.openai.com/v1/chat/completions",
        {
          ...
          messages: [
            {
              role: "system",
              content: selectedDocument?.systemPrompt || "Generate professional technical documentation."
            },
            { 
              role: "user", 
  ...
  const generatePDF = () => {
    ...
    doc.text(selectedDocument?.title || "Generated Document", 105, 30, { align: "center" });
    
    doc.setFontSize(14);
    doc.text(`Version: 1.0`, 105, 45, { align: "center" });
    doc.text(`Date: ${new Date().toLocaleDateString()}`, 105, 55, { align: "center" });
    doc.text("Generated by TechFlow AI", 105, 65, { align: "center" });
      ...
      lines.forEach(line => {
        ...
        if (selectedType === "tech-spec") {
          if (line.startsWith('# ')) {
            ...
            doc.text(wrapped, leftMargin, yPosition);
            yPosition += 7 * wrapped.length;
          }
        } 
        else if (selectedType === "srs") {
          if (line.includes(":")) {
            ...
            const wrapped = doc.splitTextToSize(value.trim(), pageWidth - 30);
            doc.text(wrapped, leftMargin + 30, yPosition);
            yPosition += 7 * wrapped.length;
          } else {
            doc.setFontSize(11);
            const wrapped = doc.splitTextToSize(line, pageWidth);
        ...
        else if (selectedType === "architecture") {
          if (line.startsWith('# ')) {
            doc.setFontSize(18);
            doc.setFont("helvetica", "bold");
            doc.text(line.substring(2), leftMargin, yPosition);
            ...
            doc.text("[Diagram placeholder - " + line.replace("[DIAGRAM]", "") + "]", leftMargin, yPosition);
            doc.rect(leftMargin, yPosition + 3, pageWidth, 40);
            yPosition += 50;
            doc.setTextColor(15, 23, 42);
          } else {
            doc.setFontSize(11);
    ...
    processContent(generatedContent);
    ...
    for (let i = 1; i <= pageCount; i++) {
      ...
      doc.text(selectedDocument?.title || "Document", 105, 290, { align: "center" });
    }
  ...
  return (
    <div className="min-h-screen flex bg-gray-50">
      ...
      >
        ...
        >
          {!sidebarCollapsed && (
            <img src="techflow_logo.png" className="w-[80%] h-auto" alt="TechFlow Logo" />
          )}
          <button
          ...
          >
            <Menu className={`w-5 h-5 text-gray-500 ${sidebarCollapsed ? "rotate-90" : ""}`} />
          </button>
        </div>
      ...
      >
        ...
        >
          <div className="flex items-center gap-4">
            {sidebarCollapsed && (
              <img src="techflow_logo.png" className="w-[11%] h-auto" alt="TechFlow Logo" />
            )}
          </div>
              ...
              <div className="bg-transparent flex flex-col gap-4 rounded-xl">
                <div className="bg-white rounded-xl shadow-md p-8">
                  <div className="flex items-center gap-3 mb-6">
                    <div className={`p-2 rounded-lg ${selectedDocument?.color}`}>
                    ...
                    <div className="p-4 bg-white">
                      {isGenerating ? (
                        ...
                        <div className={`prose max-w-none ${
                          selectedType === "tech-spec" ? "prose-blue" : 
                          selectedType === "srs" ? "prose-purple" : 
                          "prose-green"
                        }`}>
                          <div className="p-6 border border-gray-200 rounded-lg">
                            {generatedContent.split("\n").map((line, index) => {
                              if (line.startsWith("# ")) {
                                return (
                                  <h1 key={index} className="text-2xl font-bold mb-4 border-b pb-2">
                                    {line.substring(2)}
                                  </h1>
                                );
                              } else if (line.startsWith("## ")) {
                                ...
                                return (
                                  <div key={index} className="my-4 p-4 border-2 border-dashed border-gray-300 rounded-lg bg-gray-50 text-center">
                                    <p className="text-gray-500 text-sm">[Diagram: {line.replace("[DIAGRAM]", "")}]</p>
                                  </div>
                                );
                              } else if (line.trim() === "") {
                                return <br key={index} />;
                            ...
                            <div className="mt-8 pt-4 border-t text-sm text-gray-500 text-center">
                              Generated by TechFlow AI • {new Date().toLocaleDateString()}
                            </div>
                          </div>
                        </div>
                      ) : (
